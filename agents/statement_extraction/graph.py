...
def build_graph():
    ...
    graph.add_node("excel_output", node_excel_output)

    graph.add_edge(START, "validate_file")

    # Conditional edges — stop immediately if any node errors
    graph.add_conditional_edges("validate_file",   route, {"continue": "detect_format",    "end": END})
    graph.add_conditional_edges("detect_format",   route, {"continue": "extract_text",     "end": END})
    graph.add_conditional_edges("extract_text",    route, {"continue": "claude_extraction", "end": END})
    graph.add_conditional_edges("claude_extraction", route, {"continue": "validate_output", "end": END})
    graph.add_conditional_edges("validate_output", route, {"continue": "excel_output",     "end": END})

    graph.add_edge("excel_output", END)
